...
//  (a) Begin the definition of a function called product_of
...
double product_of(double items[], int num_items) {
    double product = 1.0f;
    bool isThereFiniteNumbers = FALSE;

    for (int i = 0; i < num_items; i++) {
        
        if(isfinite(items[i])) {
            product *= items[i];
            isThereFiniteNumbers = TRUE;
        }
    }
    
    if(isThereFiniteNumbers == FALSE) {
        return NAN;
    }
...
{
    double prod = product_of(x, count);
    printf("%s\n", label);
    printf("\tInput data:\n");
