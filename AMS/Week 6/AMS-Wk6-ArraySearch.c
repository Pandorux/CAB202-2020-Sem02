...
//  (a) Begin the definition of a function called find_match
...
int find_match(double values[], int num_vals, double query, double tol) {
    ...
    for(int i = 0; i < num_vals; i++) {
        ...
        double upperThres = values[i] + tol;
        double lowerThres = values[i] - tol;

        if(lowerThres <= query && query <= upperThres) {
            index = i;
        }
    }
...
int main(void)
{
    ...
    int result = find_match(items, array_size, query, error);
